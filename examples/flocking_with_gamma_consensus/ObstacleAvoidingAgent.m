...
classdef ObstacleAvoidingAgent < DoubleIntegratorAgent
    ...
    properties(Constant)
        kappa   = 0.1;  % Speed of the consensus protocol for the gamma agent
        ...
        c1_b    = 7; % P coefficient for any obstacles
    ...
    methods     
        function obj = ObstacleAvoidingAgent(id, dT, initialPos, initialVel, obstacles, q_gamma, p_gamma)
        ...
        function step(obj)
            ...
            for obst = obj.obstacles
                ...
                % Compute sigma distance between the two agents
                [pos_error, grad] = sigma_norm(qb - obj.position,...
                                          ObstacleAvoidingAgent.epsilon);
                                      
                % Calculate force on the agent from the virtual potential field
                u = u + ObstacleAvoidingAgent.c1_b...
                        * grad * phi_beta(pos_error, ObstacleAvoidingAgent.db, ObstacleAvoidingAgent.hb);
                
                % Compute position dependent factor for alignment with beta
                % agent
                b = rho_h(pos_error / ObstacleAvoidingAgent.db, ObstacleAvoidingAgent.hb);
                
                % Calculate alignment force from beta agent
                u = u + ObstacleAvoidingAgent.c2_b * b * (pb - obj.velocity);
            end
            
            % Consensus protocol for the gamma agent position & velocity
            if ~isempty(messages)
                % Calculate new gamma agent position. We use the normalized
                % Laplacian, therefore we calculate the mean of the
                % positions, not the sum
                data        = [messages.data];
                positions   = [data.q_gamma];
                speed       = [data.p_gamma];
                pos_error   = mean(obj.q_gamma - positions, 2);
                vel_error   = mean(obj.p_gamma - speed, 2);
                obj.q_gamma = obj.q_gamma - obj.kappa * pos_error;
                obj.p_gamma = obj.p_gamma - obj.kappa * vel_error;
            end
            ...
            data.q_gamma  = double(half(obj.q_gamma));
            data.p_gamma  = double(half(obj.p_gamma));
            obj.send(data)
        end
    end
end
