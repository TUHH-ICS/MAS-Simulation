...
for i = 1:length(Agents)
    ...
    % Randomly initialize the gamma agent position & velocity
    q_gamma = [60; 30] + 30 * (rand(dimension, 1) - 0.5);
    p_gamma = [3; 0] + 4 * (rand(dimension, 1) - 0.5);
    
    % Initiallize an agent with the generated initial conditions
    Agents{i} = ObstacleAvoidingAgent(Network.getId(), dT, pos, vel, obstacles, q_gamma, p_gamma);
end
Agents = [Agents{:}];
...
leech = DataLeech(Agents, steps, 'position', 'q_gamma');
...
while t < Tf
    ...
    % Save current data of all agents
    leech.save(t)
...
for k = 1:length(t_sampled)
    ...
    gamma = squeeze(sampled.q_gamma(k,:,:));
    ...
    % Draw gamma agent
    scatter(gamma(1,:), gamma(2,:), 'b+')
    
    % Draw obstacles
    for obst = obstacles
        drawCircle(obst.center, obst.radius, false);
    end
    hold off
