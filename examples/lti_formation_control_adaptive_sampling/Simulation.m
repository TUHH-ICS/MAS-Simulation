...
for i = 1:length(Agents)
    ...
    Agents{i} = AdaptiveQuadrotor(Network.getId(), pos, [i^2; 0; 0]);
end
Agents = [Agents{:}];
...
leech = DataLeech(Agents, steps, 'position', 'dT_effective');
...
figure(3)
plot(t_sampled, 2*pi ./ squeeze(sampled.dT_effective))
xlim(t_bounds);
xlabel('Time t')
ylabel('Effective sampling rate \omega')